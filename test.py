...
#python test.py -i patch_0.mat -o ./Output_folder/
import argparse
import gc
import os
...
def test_r_pnn(args):
    ...
    for band_number in range(ms.shape[1]):
        ...
        # Aux data generation
        if band.shape[2:] != pan.shape[2:]:
            print(f"Resizing pan from {pan.shape} to {band.shape[2:]}")
            pan = F.interpolate(pan, size=band.shape[2:], mode='bilinear', align_corners=False)
        ...
        for epoch in pbar:
            ...
            outputs = net(inp)
            if outputs.shape[2:] != band_lr.shape[2:]:
                # Resize outputs to match band_lr's spatial dimensions
                outputs = F.interpolate(outputs, size=band_lr.shape[2:], mode='bilinear', align_corners=False)
    ...
    file_name = basepath.split(os.sep)[-1].split('.')[0] + '_R-PNN.mat'
    save_path = os.path.join(out_dir, file_name)

    # Save the fused image
    print(f"Saving fused output to: {save_path}")
    io.savemat(save_path, {'I_MS': fused})

    # Save the training history
    history_file_name = basepath.split(os.sep)[-1].split('.')[0] + '_R-PNN_stats.mat'
    history_save_path = os.path.join(out_dir, history_file_name)
    io.savemat(history_save_path, {'loss_spec': history_loss_spec, 'loss_struct': history_loss_struct})
